...
Instance &Scene::instantiate(Mesh &mesh) const {
  CHECK(!api.graphicsLoaded(),
        "You cannot instantiate a mesh after having loaded the graphics");
...
const Renderer &Scene::registerMesh(Mesh &mesh, RenderParams &params) {
  CHECK(!api.graphicsLoaded(),
        "You cannot register a mesh after having loaded the graphics");
...
void Scene::registerComputer(ComputeParams &cparams, int dispatchX,
  ...
  CHECK(
      !api.graphicsLoaded(),
      "You cannot register a compute shader after having loaded the graphics");
